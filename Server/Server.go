...
func (s* Server)BroadcastGame() {
    ...
    // data := make([]byte, 1024)
    // var err error = nil
    entity_data_prealloc := make([]*pb.Entity, len(s.users))
    ...
    maxEventsLen := 0
    events := make([]*pb.Event, maxEventsLen)
    /*
    message := &pb.Message{
        Entities: entity_data_prealloc[:ulen],
        Events:   events,
    }
    */
    for {
...

        {
            ...
            if len(entity_data_prealloc) < ulen {
                entity_data_prealloc = make([]*pb.Entity, ulen)
                for i := 0; i< ulen; i++ {
                    entity_data_prealloc[i] = &pb.Entity{}
                }
            ...
            // Someday add adding events here. {}

            // Create a message containing entities and events
            // fmt.Println(ulen, len(entity_data_prealloc[:i]))
            message := &pb.Message{
                Entities: entity_data_prealloc[:i],
                Events: events,
            }
            // Serialise message
            // err = nil
            ...
            if datal >= 1024 {
                ...
                // fmt.Println("data length:", datal, data)
            }
