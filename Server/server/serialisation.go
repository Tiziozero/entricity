...
func oldSerialise(e *SerialisationEntityStruct) []byte {
...
// doesn't include entities that fail to oldSerialise
func oldSerialiseEntities(es []*SerialisationEntityStruct) []byte {
	data := new(bytes.Buffer)
    ...
    for _,e := range es {
        ...
        if entityData.Len() < 3 {
            fmt.Println("Failed to oldSerialise Entity inServerID")
            continue
        }

        serialisedEntityData := oldSerialise(e)
        if len(serialisedEntityData) <= 0 {
            fmt.Println("Failed to oldSerialise Entity/Entity might not have changed")
            continue
        }
        ...
        if entityData.Len() <= 3 {
            fmt.Println("Failed to oldSerialise Entity")
            continue
        } else {
...
// Returns 
func oldDeserialise(data []byte) (*SerialisationEntityStruct, error) {
...
func oldDeserialiseEntities(data []byte) ([]*SerialisationEntityStruct, error) {
    i := 0
...
func TestSingleEntity() {
    ...
    serialisedTestEntity.Write(oldSerialise(testEntity))
...
func TestMultipleEntities() {
    ...
    // Serialisation
    serialisedTestEntities := oldSerialiseEntities(testEntities)
    deserialisedTestEntities, err := oldDeserialiseEntities(serialisedTestEntities)
    if err != nil {
    ...
    if len(deserialisedTestEntities) != len(testEntities) {
        ...
        if errs != 0 {
            ...
            PrintSESs(deserialisedTestEntities, "oldDeserialiseEntities:")
        } else {
